    ...
    <div class="ff_module-bookmark ff_module-bookmark--{$data/bookmark/@type}">
      ...
      <xsl:if test="$data/bookmark/@type = 'recommended'">
          <p class="ff_module-bookmark__label ff_module-bookmark__label--meta">
              Recommended on <time datetime="{$data/bookmark/@date}"><xsl:value-of select="$data/bookmark/@date"/></time> 
              <a href="{$data/bookmark/@from_href}" class="ff_module-bookmark__link--meta"> by <xsl:value-of select="$data/bookmark/@from"/></a><xsl:text> </xsl:text>
            </p>
      </xsl:if>
    </div>
